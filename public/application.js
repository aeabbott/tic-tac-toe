...
/***/ function(module, exports, __webpack_require__) {
	...
	var authEvents = __webpack_require__(14);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var gameApi = __webpack_require__(8);
	...
	// game Restart
	var gameRestart = function gameRestart(event) {
	  $('#game-board').show();
	  $('#game-stats').show();
	  $('#game-status').show();
	  ...
	  // create game object
	  gameApi.newGame().then(gameUi.onSuccess).catch(gameUi.onError);
	};
	...
	var setMark = function setMark(event) {
	  var cells = this.id;
	  ...
	  if (moveCount < 10) {
	    ...
	    if (currentPlayer === playerOne) {
	      ...
	      .then(gameUi.onSuccess).catch(gameUi.onError);
	      // Player Two's Logic
	...
	var checkWin = function checkWin() {
	  ...
	  if (gameBoard[0] && gameBoard[0] === gameBoard[1] && gameBoard[1] === gameBoard[2] || gameBoard[3] && gameBoard[3] === gameBoard[4] && gameBoard[3] === gameBoard[5] || gameBoard[6] && gameBoard[6] === gameBoard[7] && gameBoard[6] === gameBoard[8] ||
	  // check all the columns for a win
	  gameBoard[0] && gameBoard[0] === gameBoard[3] && gameBoard[3] === gameBoard[6] || gameBoard[1] && gameBoard[1] === gameBoard[4] && gameBoard[4] === gameBoard[7] || gameBoard[2] && gameBoard[2] === gameBoard[5] && gameBoard[5] === gameBoard[8] ||
	  // check the diagonals
	  gameBoard[0] && gameBoard[0] === gameBoard[4] && gameBoard[4] === gameBoard[8] || gameBoard[2] && gameBoard[2] === gameBoard[4] && gameBoard[4] === gameBoard[6]) {
	    $('#game-board').children('').children('').off('click');
	    $('#game-status').text(currentPlayer + ' is the winner');
	    over = true;
	    gameApi.updateGameStatus(over).then(gameUi.onSuccess).catch(gameUi.onError);
	  } else if (moveCount >= 9) {
	    over = true;
	    gameApi.updateGameStatus(over).then(gameUi.onSuccess).catch(gameUi.onError);
	...
	var getGameStats = function getGameStats() {
	  ...
	  gameApi.indexGameStats().then(gameUi.onSuccessStats).catch(gameUi.onError);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateGame = function updateGame(cell, currentPlayer) {
	...
	var updateGameStatus = function updateGameStatus(over) {
	  // console.log('update game ran')
	  return $.ajax({
	    url: config.apiOrigin + '/games/' + gameStore.gameStore.game.id,
	    method: 'PATCH',
	...
	var indexGameStats = function indexGameStats() {
	  return $.ajax({
	    url: config.apiOrigin + '/games',
	    method: 'GET',
	    headers: {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var onSuccessStats = function onSuccessStats(data) {
	  // console.log('get Game stats was sucessful', data)
	  //const getFormFields = require('../../../lib/get-form-fields')
	  var games = Object.keys(data.games).length;
